#!/home/bespontoff/PycharmProjects/checkio/venv/bin/checkio --domain=py run find-friends
...
def check_connection(network, first, second):
    network = [pair.split('-') for pair in network]
    tree = set()
    tree.add(first)
    while network:
        ...
        for pair in network:
            if pair[0] in tree or pair[1] in tree:
                tree.update(pair)
                network.remove(pair)
                if second in tree: return True

        if net_size == len(network): return False
