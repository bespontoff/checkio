#!/home/bespontoff/PycharmProjects/checkio/venv/bin/checkio --domain=py run the-fastest-horse
...
def fastest_horse(horses: list) -> int:
    ...
    wins = [0 for _ in range(len(horses[0]))]
    for race in horses:
        wins[race.index(min(race))] += 1

    return wins.index(max(wins))+1
