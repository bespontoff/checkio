#!/home/bespontoff/PycharmProjects/checkio/venv/bin/checkio --domain=py run verify-anagrams
...
def verify_anagrams(first_word, second_word):
    fw = [x for x in first_word.lower() if x != ' ']
    sw = [x for x in second_word.lower() if x != ' ']
    for char in fw:
        if char in sw:
            sw.remove(char)
        else: return False
