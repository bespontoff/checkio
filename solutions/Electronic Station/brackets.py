#!/home/bespontoff/PycharmProjects/checkio/venv/bin/checkio --domain=py run brackets
...
def checkio(expression):
    ...
    exp = [ch for ch in expression if ch in '(){}[]']
    tmp = exp.copy()
    stop_ = False
    while not stop_:
        delete = False
        for i in range(len(exp)-1):
            if exp[i] in brackets.keys() and exp[i+1] == brackets[exp[i]]:
                tmp.pop(i)
                tmp.pop(i)
                delete = True
                break
        exp = tmp.copy()
        if not delete: stop_ = True
