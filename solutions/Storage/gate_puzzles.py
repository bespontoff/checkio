#!/home/bespontoff/PycharmProjects/checkio/venv/bin/checkio --domain=py run gate-puzzles
...
def find_word(message):
    ...
    words = message.split()    
    words_enum =  {x:words[x] for x in range(len(words))}    
    table = []    
    ...
    for i in range(len(words)):
        ...
        for j in range(len(words)):
            ...
            if i == j: row.append(0)
                ...
                if words[i][:1] == words[j][:1]: tax += 10
                if words[i][-1:] == words[j][-1:]: tax += 10
                if len(words[i]) <= len(words[j]): tax += (len(words[i])/float(len(words[j])))*30
                elif len(words[j]) <= len(words[i]): tax += (len(words[j])/float(len(words[i])))*30
                ...
                uniq = word1
                uniq.update(word2)
                tax += (len(common) / float(len(uniq))) * 50
                row.append(tax)
    ...
    mid = []
    for row in table:        
        mid.append(sum(row)/float(len(row)-1))
    max_ = max(mid)    
    while mid.count(max_) > 1:
        mid[mid.index(max_)] = 0
        
    return words_enum[mid.index(max(mid))]
